...
bool inBorder(cv::Point2f & pt, int & row, int & col)
{
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time, bool pub_this_frame, int equalize)
{
    cv::Mat img;
    TicToc t_r;
    cur_time = _cur_time;
    ...
    if (cur_pts.size() > 0)
    {
        ...
        for (int i = 0; i < int(forw_pts.size()); i++)
            if (status[i] && !inBorder(forw_pts[i], row, col))
                status[i] = 0;
