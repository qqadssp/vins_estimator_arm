...
void Estimator::setParameter(Eigen::Vector3d & TIC, Eigen::Matrix3d & RIC, double TD)
{
    tic = TIC;
    ric = RIC;
    td = TD;
...
void Estimator::setFeatureManager(double INIT_DPETH, double MIN_PARALLAX, double FOCAL_LENGTH)
{
	focal_length = FOCAL_LENGTH;

    f_manager.init_depth = INIT_DPETH;
    f_manager.min_parallax = MIN_PARALLAX;

    f_manager.window_size = WINDOW_SIZE;
...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    {
        pre_integrations[frame_count] = new IntegrationBase{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count], acc_n, acc_w, gyr_n, gyr_w, G};
    }
    if (frame_count != 0)
    {
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double header)
{
    ...
    tmp_pre_integration = new IntegrationBase{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count], acc_n, acc_w, gyr_n, gyr_w, G};
    ...
    if(estimate_extrinsic == 2)
    {
        ...
        if (frame_count != 0)
        {
            ...
            Matrix3d calib_ric;
            initial_ex_rotation.window_size = WINDOW_SIZE;
...
bool Estimator::visualInitialAlign()
{
    ...
    VectorXd x;
    //solve scale
    bool result = VisualIMUAlignment(all_image_frame, Bgs, g, x, tic, G, WINDOW_SIZE);
    if(!result)
    {
    ...
    Vector3d TIC_TMP;
    TIC_TMP.setZero();
    f_manager.triangulate(Ps, Rs, TIC_TMP, ric);
...
void Estimator::double2vector()
{
    ...
    tic = Vector3d(para_Ex_Pose[0],
                      para_Ex_Pose[1],
                      para_Ex_Pose[2]);
    ric = Quaterniond(para_Ex_Pose[6],
                         para_Ex_Pose[3],
                         para_Ex_Pose[4],
                         para_Ex_Pose[5]).toRotationMatrix();


    VectorXd dep = f_manager.getDepthVector();
    for (int i = 0; i < f_manager.getFeatureCount(); i++)
        dep(i) = para_Feature[i];
...
void Estimator::optimization()
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if (pre_integrations[j]->sum_dt > 10.0)
            continue;
        IMUFactor* imu_factor = new IMUFactor(pre_integrations[j], G);
    ...
    if(relocalization_info)
    {
        ...
        for (auto &it_per_id : f_manager.feature)
        {
            ...
            {   
                while((int)match_points[retrive_feature_index].id < it_per_id.feature_id)
                {
                    retrive_feature_index++;
                }
                if((int)match_points[retrive_feature_index].id == it_per_id.feature_id)
                {
                    Vector3d pts_j = Vector3d(match_points[retrive_feature_index].x, match_points[retrive_feature_index].y, 1.0);
                    Vector3d pts_i = it_per_id.feature_per_frame[0].point;
                    
                    ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j);
                    problem.AddResidualBlock(f, loss_function, para_Pose[start], relo_Pose, para_Ex_Pose, para_Feature + feature_index);
                    retrive_feature_index++;
                }     
            }
        }
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
...

        {
            ...
            {
                IMUFactor* imu_factor = new IMUFactor(pre_integrations[1], G);
                ...
                for (auto &it_per_frame : it_per_id.feature_per_frame)
                {
                    ...
                    if (estimate_td)
                    {
                        ...
                        ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(f_td, loss_function,
                                                                                        vector<double *>{para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose, para_Feature + feature_index, para_Td},
                                                                                        vector<int>{0, 3});
                        marginalization_info->addResidualBlockInfo(residual_block_info);
                    }
                    else
...
void Estimator::slideWindow()
{
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        if (frame_count == WINDOW_SIZE)
        {
            ...
            pre_integrations[WINDOW_SIZE] = new IntegrationBase{acc_0, gyr_0, Bas[WINDOW_SIZE], Bgs[WINDOW_SIZE], acc_n, acc_w, gyr_n, gyr_w, G};
...
void Estimator::setReloFrame(double _frame_stamp, int _frame_index, vector<StRelocPoint> &_match_points, Vector3d _relo_t, Matrix3d _relo_r)
{
